...
export class DeduplicationStage implements INotificationStage {
  ...
  async execute(
    ...
    try {
       ...
       */
      const alreadyDelivered = await this.cacheService.exists(dedupKey);
       ...
       */
      const lockResult = await this.cacheService.set(
        processingKey,
        "processing",
    ...
    try {
      ...
      if (processingLockAcquired) {
        try {
          await this.cacheService.delete(processingKey);
        } catch (error) {
          this.logger.error({
